...
```
jq -r '.skills.entries["gh-issues"].apiKey // empty' "${OPENCLAW_CONFIG_PATH:-${OPENCLAW_STATE_DIR:-$HOME/.openclaw}/openclaw.json}"
```
...
- If curl returns an HTTP 401 or 403 → stop and tell the user:
  > "GitHub authentication failed. Please check your apiKey in the OpenClaw dashboard or in the active OpenClaw config at $OPENCLAW_CONFIG_PATH, falling back to $OPENCLAW_STATE_DIR/openclaw.json."
...
export GH_TOKEN=$(node -e "const fs=require('fs');const p=process.env.OPENCLAW_CONFIG_PATH || ((process.env.OPENCLAW_STATE_DIR || process.env.HOME + '/.openclaw') + '/openclaw.json');const d=JSON.parse(fs.readFileSync(p,'utf8'));console.log(d.skills?.entries?.['gh-issues']?.apiKey||'')")
