...
REPO_ROOT="$(resolve_repo_root)" || {
  echo "ERROR: could not locate an OpenClaw checkout with node_modules/tsx for wacli-recent-reply." >&2
  echo "Set OPENCLAW_FORK_ROOT to your OpenClaw checkout and run pnpm install there." >&2
  exit 1
}

cd "$REPO_ROOT"
exec node --import tsx "$REPO_ROOT/skills/wacli/scripts/wacli-recent-reply.ts" "$@"
