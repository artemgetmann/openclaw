...
struct CLIInstallerTests {
    ...
    @Test func `installed location rejects consumer wrapper when payload is missing`() throws {
        let fm = FileManager()
        let root = fm.temporaryDirectory.appendingPathComponent(
    ...
    @Test @MainActor func `consumer ensureInstalledIfNeeded repairs from bundled app runtime`() async throws {
        ...
        let result = await TestIsolation.withIsolatedState(
            env: [
        ...
        {
            let result = await CLIInstaller.ensureInstalledIfNeeded(bundle: bundle, fileManager: fm)
            let installLocation = ConsumerRuntime.installPrefixURL.appendingPathComponent("bin/openclaw").path
            switch result {
        ...
        if case .failed = result {
            Issue.record("Bundled repair returned failure")
        }
    }
    ...
    private func makeConsumerBundle(resourceRoot: URL) throws -> Bundle {
        ...
        guard let bundle = Bundle(path: bundleURL.path) else {
            throw NSError(
                ...
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Could not load temp bundle"])
        }
        return bundle
    }
        ...
        fileManager: FileManager) throws
    {
        guard let resourceURL = bundle.resourceURL else {
            throw NSError(
                domain: "CLIInstallerTests",
                code: 2,
                userInfo: [NSLocalizedDescriptionKey: "Temp bundle missing resources"])
        }
