...
final class GatewayProcessManager {
    ...
    private func enableLaunchdGateway(preferKickstart: Bool = false) async {
        ...
        // Only force a hard restart when we explicitly know recovery is required.
        // A pre-existing plist is normal on app launch; treating that as a restart trigger
        // creates exactly the churn we want to avoid:
        // 1. app opens,
        // 2. launchd gateway begins booting,
        // 3. app sees an existing plist and kickstarts it,
        // 4. launchd SIGTERMs the still-starting gateway,
        // 5. health/UI now oscillate even though the runtime itself is fine.
        //
        // Let the normal enable/start path reuse the existing launchd registration. Reserve the
        // destructive restart path for explicit recovery after we have evidence the live listener
