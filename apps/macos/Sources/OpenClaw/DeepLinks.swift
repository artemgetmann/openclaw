...
final class DeepLinkHandler {
    ...
    func handle(url: URL) async {
        ...
        guard !AppStateStore.shared.isPaused else {
            ...
            self.presentAlert(title: "\(appName) is paused", message: "Unpause \(appName) to run agent actions.")
            return
        }
    ...
    private func handleAgent(link: AgentDeepLink, originalURL: URL) async {
        ...
        if !allowUnattended {
            ...
            guard self.confirm(title: "Run \(AppFlavor.current.appName) agent?", message: body) else { return }
        }
