...
extension ChannelsStore {
    ...
    func captureTelegramFirstDirectMessage() async {
        ...
        do {
            ...
            // Drop the in-progress UI state immediately once the config write
            // succeeds. The gateway can take another beat to reprobe Telegram, but
            // the setup flow itself is already done and should stop spinning.
            self.telegramSetupPhase = .idle
            self.telegramSetupWaitingForDM = false
