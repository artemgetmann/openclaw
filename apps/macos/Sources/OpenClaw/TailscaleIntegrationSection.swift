...
struct TailscaleIntegrationSection: View {
    ...
    private var authFields: some View {
        ...
        Text("Stored in \(AgentWorkspace.displayPath(for: OpenClawPaths.configURL)). Prefer OPENCLAW_GATEWAY_PASSWORD for production.")
            .font(.caption)
            .foregroundStyle(.secondary)
    ...
    private func applySettings() async {
        ...
        if self.connectionMode == .local, !self.isPaused {
            self.statusMessage = "Saved to \(AgentWorkspace.displayPath(for: OpenClawPaths.configURL)). Restarting gateway…"
        } else {
            self.statusMessage = "Saved to \(AgentWorkspace.displayPath(for: OpenClawPaths.configURL)). Restart the gateway to apply."
        }
