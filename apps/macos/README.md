...
entrypoint exits `75`; reconcile that owner before starting a new release turn.
Never treat an unattended shell wait as proof that Codex will remain active or
report the eventual result. Never improvise a `ps`-scanning `SIGSTOP`/`SIGKILL`
guard: the canonical lock never signals a process. The wrapper's true
