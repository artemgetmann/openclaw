...
#
# Secrets should stay in Keychain whenever possible. This optional env file is
# only a deterministic bridge for non-secret release settings and secret file
# pointers, so fresh worktrees can discover the same release inputs without
...
Store notary credentials in Keychain with:
