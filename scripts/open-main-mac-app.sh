...
APP_PATH="${1:-$ROOT_DIR/dist/OpenClaw.app}"
INFO_PLIST="$APP_PATH/Contents/Info.plist"
EXPECTED_NAME="OpenClaw"
EXPECTED_BUNDLE_ID="ai.openclaw.mac.debug"
EXPECTED_VARIANT="standard"

if [[ ! -f "$INFO_PLIST" ]]; then
  echo "ERROR: main app bundle not found: $APP_PATH" >&2
  echo "Run scripts/package-mac-app.sh first." >&2
  exit 1
fi
...
if [[ "$actual_name" != "$EXPECTED_NAME" || "$actual_bundle_id" != "$EXPECTED_BUNDLE_ID" || "$actual_variant" != "$EXPECTED_VARIANT" ]]; then
  echo "ERROR: refusing to open a stale/non-debug main bundle." >&2
...
pkill -f '/Applications/OpenClaw\.app/Contents/MacOS/OpenClaw' || true
pkill -f "$APP_PATH/Contents/MacOS/OpenClaw" || true
open -n "$APP_PATH"

echo "Opened main app:"
echo "  path=$APP_PATH"
