...
DEFAULT_SPARKLE_PUBLIC_ED_KEY="AGCY8w5vHirVfGGDGc8Szc5iuOqupZSh9pMj/Qs67XI="
...
verify_required_capabilities_manifest() {
  ...
  # Cached and smoke-reused runtimes must carry the same app-owned payload as a
  # fresh package. Otherwise a cache hit can silently regress clean installs
  # back to "gog must already exist somewhere on the user's PATH."
...
write_transitional_memory_template_if_missing() {
  ...
  # Main does not yet carry Consumer's MEMORY.md template. Keep this packaging
  # slice self-contained by staging the transitional template into the bundle
  # without editing docs/reference/templates outside the assigned ownership.
  cat > "$template_dir/MEMORY.md" <<'EOF'
---
title: "MEMORY.md Template"
summary: "Workspace template for long-term consumer memory"
read_when:
...
copy_consumer_workspace_templates() {
  local template_src="$1"
  local template_dest="$2"
  local context_label="$3"

  if [[ ! -d "$template_src" ]]; then
    echo "ERROR: ${context_label} source directory missing: $template_src" >&2
    return 1
  fi
  ...
  write_transitional_memory_template_if_missing "$template_dest"
  verify_required_workspace_templates "$template_dest" "$context_label"
}
...
prepare_bundled_consumer_runtime() {
  ...
  copy_consumer_workspace_templates \
    "$bundled_template_src" \
    "$bundled_template_dest" \
    "bundled consumer runtime workspace templates"
...
copy_consumer_workspace_templates \
  "$TEMPLATE_SRC" \
  "$TEMPLATE_DEST" \
  "packaged consumer app workspace templates"
