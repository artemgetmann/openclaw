...
- Progress UX v2: the same lane now keeps model-authored progress narration as a retained progress bubble and sends the final answer as a separate bubble, instead of pasting progress into the final answer. Follow-up duplicate-final fix now handles late partials that already contain `progress + final` by restoring the clean retained progress prefix before final message rotation, and the boundary formatter now separates acronym-led follow-up progress like `now.IANA`. Focused proof passed: `pnpm exec vitest run extensions/telegram/src/bot-message-dispatch.test.ts extensions/telegram/src/lane-delivery.test.ts src/infra/telegram-live-runtime-helpers.test.ts --pool=forks --maxWorkers=1` passed 3 files / 125 tests.
...
- Complex delayed reminder parity follow-up: the inline reminder shortcut now recognizes the user's natural `in one minute ...` phrasing and stores the delayed `agentTurn` payload as an execution instruction instead of a reminder-shaped note. Focused local proof passed: `pnpm exec vitest run src/auto-reply/reply/reminder-intent.test.ts src/auto-reply/reply/get-reply-inline-actions.skip-when-config-empty.test.ts src/cron/isolated-agent/run.complex-reminder.test.ts --pool=forks --maxWorkers=1` passed 3 files / 11 tests, including a Claude CLI handoff assertion that the complex browser-style task reaches `runCliAgent` intact. Live Telegram/browser proof was not run in this lane.
- Product-level complex reminder correction: direct `cron add` was not accepted as final proof. The first local product probes showed `openclaw agent` and `gateway call chat.send` could fall through to Claude because product chat prefixes the body with a timestamp before reminder parsing. The parser now strips that timestamp prefix before intent detection. Corrected local product proof used `gateway call chat.send` with exact message `in one minute go to my Twitter profile, click on the first post, and check the first comment`; it created cron job `89cb08ec-0a4b-41e6-aded-fa7f774a24eb`, then fired as `provider=claude-cli model=sonnet` and completed the X/browser read. Result summary captured the first comment from `@vadym_petryshyn`: `That shift from IG to X works because daily posting here compounds faster than polished static grids`. Delivery was not proven in this local path because the internal chat session had no Telegram target (`Delivering to Telegram requires target <chatId>`). Tester Telegram could not be driven from this lane because MTProto `TELEGRAM_API_ID`/`TELEGRAM_API_HASH` were unavailable.
- Tester Telegram reminder delivery follow-up: passed on `@Artem_jarvis_email_bot` using the natural prompt `in one minute go to my Twitter profile, click on the first post, and check the first comment`. Runtime was isolated to worktree `/Users/user/Programming_Projects/openclaw/.worktrees/claude-cli-reminder-parity-20260516`, branch `codex/claude-cli-reminder-product-parity-20260516`, port `24587`, with default model `claude-cli/sonnet`. User message `49433` produced scheduling reply `49434`; cron job `a6a44bb6-90e6-4bd2-9147-c53a75433942` stored an `agentTurn` payload with Telegram delivery target `1336356696`; delayed run completed with `provider=claude-cli model=sonnet`, `deliveryStatus=delivered`, and final Telegram message `49435`. Delivered first comment was from Arthur Katcher (`@arthurkatcher`): `damn that's amazing, are you living there as a digital nomad or traveling through?`. Test-only runtime and Chrome sidecar were stopped, and ports `24587`, `24589`, and `24598` were verified closed.
- Skills product proof follow-up: passed via `node openclaw.mjs agent --local --agent productcheck --message "$PROMPT" --json --timeout 240` on `claude-cli/sonnet` with the exact Reddit URL `https://www.reddit.com/r/codex/comments/1swq4g4/what_theme_do_you_use_in_codex_app/`. The run exposed the `reddit` OpenClaw skill in the prompt, Claude read the skill file, used `skills/reddit/scripts/reddit.mjs comments`, summarized the post/comments, and confirmed in a same-session follow-up that it used the Reddit skill.
...
- Root cause found in the skills parity lane: Claude CLI received the compact `<available_skills>` inventory, but the bridge-safe prompt did not make the OpenClaw skill contract explicit enough for Claude's separate native skill registry. The fix tells Claude to select matching OpenClaw skills from `<available_skills>`, read their `SKILL.md` locations, prefer them over generic web/search behavior before saying a skill is missing, and allows Claude's native `Read` tool alongside `mcp__openclaw__*`.
