...
- Progress UX v2: the same lane now keeps model-authored progress narration as a retained progress bubble and sends the final answer as a separate bubble, instead of pasting progress into the final answer. Follow-up duplicate-final fix now handles late partials that already contain `progress + final` by restoring the clean retained progress prefix before final message rotation. Focused proof passed: `pnpm exec vitest run extensions/telegram/src/bot-message-dispatch.test.ts extensions/telegram/src/lane-delivery.test.ts src/infra/telegram-live-runtime-helpers.test.ts --pool=forks --maxWorkers=1` passed 3 files / 124 tests.
