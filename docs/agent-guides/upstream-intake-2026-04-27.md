...
| 7    | candidate | `4f00b76925`, `4bc46ccfed` context and compaction hardening            | Tightens context limits, bounds memory excerpts, and caps compaction reserve for small models.                                       | `src/agents/agent-scope*`, `src/agents/pi-embedded-runner/**`, `src/agents/session-tool-result-guard*`, memory files                                                                                        | Long personal-agent sessions degrade without good context hygiene.                                       | port later, soon | Start with tool-result truncation and compaction constants. Avoid broad memory-core churn unless tests require it.                                | Pending                                         |
| 8    | candidate | `77719899f3`, `4ee537a04a` gateway/node-host recovery                  | Refreshes stale embedded service tokens and keeps node-host recovering after gateway restarts.                                       | `src/cli/daemon-cli/install.ts`, `src/gateway/client.ts`, `src/node-host/runner.ts`, tests                                                                                                                  | Relevant to macOS runtime reliability, but must respect consumer runtime isolation.                      | port later       | Port token refresh test first, then node-host recovery. Review against LaunchAgent/profile isolation.                                             | Pending                                         |
...
| ---------- | --------- | ---------------------------------------- | ---------------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 2026-05-02 | validated | Model auth/discovery truthfulness subset | pending commit   | `pnpm exec vitest run ...` aggregate targeted suite: 13 files, 253 tests passed | Ported agent-scoped auth commands and OpenAI Codex metadata/discovery normalization. `#71985` and gateway auth-status pieces had no direct local target. |
| 2026-05-02 | validated | Telegram reply/progress UX               | pending commit   | Aggregate targeted suite: 13 files, 253 tests passed                            | Ported native quote reply propagation, quote rejection fallback, and fork-equivalent progress fallback coverage.                                         |
| 2026-05-02 | validated | Visible assistant text cleanup           | pending commit   | Aggregate targeted suite: 13 files, 253 tests passed                            | Ported standalone function tool-call stripping and namespaced `antml:thinking` stripping/promoting.                                                      |
| 2026-05-02 | validated | Host tilde path handling                 | pending commit   | Aggregate targeted suite: 13 files, 253 tests passed                            | Ported OS-home tilde expansion for host read/write/edit and edit recovery.                                                                               |
