...
- Same-user RC onboarding is accepted. The restored Telegram copy, delayed
  ChatGPT recovery help, first-message replay, and `Verify Telegram` recovery
  path passed the focused same-user run and targeted tests.
- Telegram UX issues from that run are no longer release blockers. Do not keep
  polishing same-user onboarding unless the clean-user Gate2 lane proves a real
  regression.
- Sparkle implementation and release assets are merged on `main`, but end-to-end
  update proof is still pending.
- Clean-user Gate2 proof is the next real release blocker.
- Final release still requires a notarized package plus Gatekeeper proof from
  the synced final head.
...
| -------- | ------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------- | -------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| P0       | AI access onboarding must be coherent enough that the first Telegram DM can complete a useful task.                             | Same-user RC accepted; clean-user Gate2 proof next.                                                                   | Onboarding redesign lane / tmux pane 1 | Same-user proof now reaches Telegram verification and a bot reply. Re-run with the distinct Gate2 identity on a clean macOS user before calling the release lane done. |
...
| P0       | Recut/package from current `main` after the P0 onboarding fixes before claiming source polish ships to the 4-5 waiting testers. | Open; blocked on clean-user Gate2 plus final notarized artifact proof.                                                | Release/package lane after P0          | Sync PR #840 with `main`, build the final notarized DMG/ZIP from that head, verify Gatekeeper, and do not claim the old DMG contains the current onboarding fixes.     |
...
| P1       | Full app run-through after P0 onboarding/account/AI-access and Telegram setup land.                                             | Same-user run accepted; clean-user Gate2 run still pending.                                                           | Post-P0 proof lane                     | The remaining proof is a clean-user run with isolated Gate2 identity, permissions, onboarding, and log collection without touching the default gateway.                |
...
| P1       | Full Sparkle update-cycle proof.                                                                                                | Open; implementation/assets are merged on `main`, but proof is still pending.                                         | Release/package lane                   | Prove download, verify, install, relaunch, and state preservation against the current release line after the final notarized artifact exists.                          |
