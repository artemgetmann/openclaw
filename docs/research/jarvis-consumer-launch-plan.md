...
- [x] Produce local Jarvis trusted-tester artifacts from current `main`:
      ...
      but `/Applications/Jarvis.app` still reports older build `14d2624bb8`;
      install-over-current and final installed-app smoke are deferred to the
      next chat.
- [ ] Install `/Users/user/Programming_Projects/openclaw/Jarvis.dmg` over the
      current app, verify the Permissions tab shows only the four core
      permissions, then run a clean macOS user install smoke. After it passes,
      sending `Jarvis.dmg` to the 3 trusted waiting testers is allowed unless
      Artem explicitly chooses to send earlier.
...
| ------------------------------------------------ | ------------------------------------------------ | --------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Final package through deterministic release lane | Release lane from current `main`                 | Artifact done; install smoke next | Final 2026-05-14 trusted-tester `Jarvis.dmg` exists at `/Users/user/Programming_Projects/openclaw/.worktrees/jarvis-package-recut-20260514/dist/Jarvis.dmg` and `/Users/user/Programming_Projects/openclaw/Jarvis.dmg`, built from commit `ab9c3c1ca1`; the DMG is Gatekeeper-accepted as Notarized Developer ID. Current `/Applications/Jarvis.app` still reports older build `14d2624bb8`; the next chat owns install-over-current, Permissions tab verification, and clean-user smoke. |
...
| Clean macOS user install smoke                   | Release/GUI smoke lane                           | Recommended next before send      | Install `/Users/user/Programming_Projects/openclaw/Jarvis.dmg` over the current app, verify the Permissions tab shows only Screen Recording, Accessibility, Automation, and Location in the primary list, then run a clean macOS user install smoke before sending to the 3 trusted waiting testers. After it passes, sending `Jarvis.dmg` to those testers is allowed unless Artem explicitly chooses earlier.                                                                           |
...
- [ ] Install the final DMG over current `/Applications/Jarvis.app`, verify
      Permissions tab grouping, then run a clean macOS user install smoke before
      sending to the 3 trusted waiting testers. After it passes, send is allowed
      unless Artem explicitly chooses earlier.
- [ ] Run the next release lane with App Store Connect API key auth plus async
