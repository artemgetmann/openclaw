...
1. signed/notarized downloadable app for the 3 trusted waiting testers — final
   ...
   Gatekeeper-accepted as Notarized Developer ID, `/Applications/Jarvis.app`
   matches `ab9c3c1ca1`, visible name/icon are Jarvis, installed runtime
   takeover is green, and Channels shows Telegram live/verified as
   `@Jarvis_cl4w_bot`; public `v2026.3.15` release assets now include
   `Jarvis.dmg`, `Jarvis.zip`, and `jarvis-appcast.xml`; this trusted-tester
   recut keeps `ai.openclaw.consumer.mac`/OpenClaw runtime identity by
   deliberate 80/20 decision
...
- [x] Produce local Jarvis trusted-tester artifacts from current `main`:
      ...
      `/Applications/Jarvis.app` matches `ab9c3c1ca1`, visible
...
- [x] Produce notarized Jarvis trusted-tester app/DMG/ZIP/appcast from current
      ...
      `/Users/user/Programming_Projects/openclaw/.worktrees/jarvis-package-recut-20260514/dist/Jarvis.dmg`
      and `/Users/user/Programming_Projects/openclaw/Jarvis.dmg` from commit
      `ab9c3c1ca1`; installed app/runtime/Channels smoke passed. Public
      `v2026.3.15` now includes `Jarvis.dmg`, `Jarvis.zip`, and
      `jarvis-appcast.xml`.
