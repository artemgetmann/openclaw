...
- [x] Fix post-launch app-bundle mutation before broader distribution. Root
      ...
      and macOS `ConsumerBundledRuntimeTests` plus
      `GatewayLaunchAgentManagerTests` passed per suite on 2026-05-16. No live
      `/Applications/Jarvis.app` mutation was performed.
...
| Post-launch app-bundle mutation                  | Packaging/runtime lane before public-ish launch  | Fixed in branch, PR validation required | Fixed by treating bundled app resources as seed-only, running packaged gateway identity from the seeded Application Support runtime, and sending managed `acpx` installs to `$OPENCLAW_STATE_DIR/cache/extensions/acpx`. Targeted `acpx` Vitest and macOS bundled-runtime/launch-agent suites passed on 2026-05-16 without touching `/Applications/Jarvis.app`. Keep PR open unless package smoke proves the signed bundle stays unchanged after launch.                                          |
