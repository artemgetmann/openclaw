...
blocked at the LaunchAgent ownership gate. PR #645 merged as
`ab9c9de42df9bb8058d75528f9e6dbb77bef7a6c`, fixing Telegram outbound bot
reply activity telemetry.
...
PR #650 is open at `https://github.com/artemgetmann/openclaw/pull/650` on
branch `codex/launchagent-packaged-takeover-20260509`, commit
`d9b38bf7aef2106edae9b80fa28a509e689e1f6d`. It makes packaged consumer
LaunchAgent ownership prefer
`/Applications/OpenClaw.app/Contents/Resources/OpenClawRuntime/openclaw/dist/index.js`
when the bundled runtime exists while preserving the source/dev fallback.
Validation so far: `swift test --filter GatewayLaunchAgentManagerTests` passed
21 tests, `swift test --filter GatewayProcessManagerTests` passed 3 tests, and
`git diff --check` passed.

Do not claim Channels GUI pass until #650 or equivalent is merged, the installed
app is rebuilt/relaunched from current `main`, loaded LaunchAgent
`ProgramArguments[1]` is packaged, `/healthz` is live, and the GUI Channels
Telegram verified state is proven.
...
| Gateway ownership / launch behavior | Blocked on #650 proof          | Shared gateway/service ownership is canonical in `main`, with takeover guardrails and canonical env fixes merged. #597 prevents the consumer app from stopping an already-healthy canonical gateway during setup/attach paths. #599 keeps watchdog recovery shallow. #613/#614 allow packaged app ownership and launchd context. #615 prefers the bundled runtime root. #620/#625/#638 repaired stale LaunchAgent, source-attach, and service-version drift cases. New blocker: `/Applications/OpenClaw.app` can run while `ai.openclaw.gateway` remains attached to `/Users/user/Programming_Projects/openclaw/dist/index.js`; `/healthz` can still be live and is not enough. PR #650 opens the packaged-runtime preference fix and focused Swift tests plus `git diff --check` pass. | Merge #650, rebuild/relaunch installed app from current `main`, prove LaunchAgent `ProgramArguments[1]` is packaged, then prove `/healthz`. |
| Consumer macOS shell parity         | Blocked on installed GUI proof | Consumer setup shell pieces were ported into `main`: browser setup, readiness, permissions, Telegram setup card/state/verifier, bundled runtime/bootstrap, and packaging entrypoints. Existing-user and isolated fresh-user smokes passed. #645 merged as `ab9c9de42df9bb8058d75528f9e6dbb77bef7a6c` and fixes Telegram outbound bot reply activity telemetry. Installed-app GUI proof is now blocked by LaunchAgent packaged-runtime ownership, not Telegram telemetry.                                                                                                                                                                                                                                                                                                                | After #650 merges and launchd points at the packaged runtime, rerun GUI Channels Telegram verified-state smoke.                             |
...
### 1. Merge #650 and rerun installed-app Channels smoke

PR #645 merged as `ab9c9de42df9bb8058d75528f9e6dbb77bef7a6c`, so Telegram
outbound activity telemetry is no longer the current blocker. PR #650
(`https://github.com/artemgetmann/openclaw/pull/650`) is the active fix for the
installed-app LaunchAgent staying on the source-checkout runtime while
`/Applications/OpenClaw.app` is running.

After #650 merges, rebuild/relaunch the installed app from current `main`.
Pass only if loaded LaunchAgent `ProgramArguments[1]` is
`/Applications/OpenClaw.app/Contents/Resources/OpenClawRuntime/openclaw/dist/index.js`,
`/healthz` is live, and the GUI Channels Telegram verified state is proven.
...
Channels tab first-task verifier state, and #645 fixes the outbound activity
telemetry gap. The already-published `v2026.3.15` artifacts do not contain
#638/#634/#645. Sparkle/final public artifacts must be rebuilt from current
`main` after #650 if it merges and the installed-app proof passes.
...
`ab9c9de42df9bb8058d75528f9e6dbb77bef7a6c`. The latest default installed app
proof is blocked earlier because LaunchAgent stayed on the source-checkout
runtime. PR #650 is the active repair; do not claim installed-app GUI proof
until the LaunchAgent points at the packaged runtime and the Channels tab
auto-promotes.
