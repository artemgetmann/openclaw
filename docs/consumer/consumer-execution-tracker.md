...
- Scope:
  - practical consumer demo distribution only
  - no broad release engineering or appcast rollout
- Repo-side fixes landed in this lane:
  - removed duplicated `MEMORY.md` declarations/templates in the mac app workspace bootstrap so release packaging no longer fails in `AgentWorkspace.swift`
...
- Concrete findings:
  ...
  - the practical blocker for real notarized distribution on this Mac is Apple-side, not app-side:
    ...
    - no notary auth is configured in env (`NOTARYTOOL_PROFILE`, `NOTARYTOOL_KEY`, `NOTARYTOOL_KEY_ID`, `NOTARYTOOL_ISSUER` were all unset during this pass)
...
- Decision:
  - treat Developer ID certificate availability and notary credentials as the remaining blockers for notarized/stapled demo distribution
  - do not treat consumer app assembly, consumer bundle identity, or DMG creation as blockers anymore
  - keep local/manual-trust demo packaging available via `SKIP_NOTARIZE=1` until the Apple-side requirements are in place
- Clean-machine / first-open expectation after this pass:
  ...
  - seamless direct-download expectation remains blocked until:
    ...
    - stapling succeeds
- Exact verification steps and results from this lane:
  - `security find-identity -p codesigning -v`
    - result: only `Apple Development: artem.getman@icloud.com (9642P4S39P)` was available
  - `for k in NOTARYTOOL_PROFILE NOTARYTOOL_KEY NOTARYTOOL_KEY_ID NOTARYTOOL_ISSUER SIGN_IDENTITY; do ...; done`
    - result:
