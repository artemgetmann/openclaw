...
| Consumer packaging from main          | Completed for v2026.3.14 | Main produced the signed/notarized `v2026.3.14` release from `41f2868ad7a2f174aad7e385f0c28efe81f816c0`. Codesign stale-signature and DMG conversion failures are fixed. Packaging copies DMG/ZIP/dSYM ZIP handoff artifacts to the main checkout by default. Sparkle consumer release gates are in place. GitHub release assets are `OpenClaw.dmg`, `OpenClaw.zip`, `openclaw-consumer-appcast.xml`, and `OpenClaw-2026.3.14.dSYM.zip`. A branch artifact from `fe05c860a87411709effdf83050c1bae2ad601cd` proved the post-#625 repair locally before #625 was squash-merged.                                                                                                                                                                                                         | Sparkle/notarized public rebuild is owned by the separate release lane. This lane only needs final artifact provenance after that lane publishes.    |
...
1. Wait for the separate Sparkle/release lane to rebuild and publish the next
   public artifact from `main` at
   `5ce43d8538e223db4390733c01611f0684d12541` or newer.
2. Run installed-release smoke from that post-#625 public DMG/ZIP artifact.
...
key/feed handling, and the next notarized public rebuild are owned by the
separate Sparkle/release lane. This tracker should consume that lane's published
artifact provenance instead of duplicating the work here.
...
- Current-main caveat: `v2026.3.14` is a real release but not current `main`
  ...
  `bed0de66fd3011b9c8a2d4f63b0ac59a9ef1b0a1`, #620, and the later #625 merge
  at `5ce43d8538e223db4390733c01611f0684d12541`.
...
- Stop saying Developer ID notarization remains open for `v2026.3.14`; the
  ...
  The next notarized post-#625 rebuild is a separate Sparkle/release-lane task.
