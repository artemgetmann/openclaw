...
| Gateway ownership / port              | Debug proof passed; GUI blocked | Default gateway is canonical `ai.openclaw.gateway` on `18789`. #597/#599/#613/#614/#615/#620/#625/#638 landed the known packaged ownership and repair pieces. #650 merged as `4d01d8af6e4be91c4009818314c64c1e8838bb57` and fixed stale entrypoint mismatch detection, but live proof after #650 still failed because launchd install wrote source-checkout `ProgramArguments[1]`. #651 merged as `f2382618524142ea0023d9d3a2fbff9cc61d8f61` and preserves packaged `OpenClawRuntime` roots for gateway install/restart and `OPENCLAW_FORK_ROOT`. Isolated debug proof from `origin/main` at `f238261852` showed loaded `ProgramArguments[1]` under `dist/OpenClaw (gui-verify-20260508).app/.../OpenClawRuntime/openclaw/dist/index.js`, and `/healthz` on `127.0.0.1:39388` returned live. | GUI Channels proof remains blocked by Computer Use/TCC Apple event error `-10000`.                        |
| LaunchAgent/service behavior          | Debug proof passed              | Shared service install/restart behavior and takeover guardrails are in `main`. #638 repairs stale `OPENCLAW_SERVICE_VERSION` env after Sparkle update. #650 repaired stale entrypoint mismatch detection. #651 repaired the install/root side. Validation before #651 merge: `swift test --filter GatewayLaunchAgentManagerTests` passed 22 tests, `swift test --filter CommandResolverTests` passed 14 tests, and `git diff --check` passed. Fast rebuild/relaunch after #651 with `ALLOW_SINGLE_ARCH_CONSUMER_SMOKE=1 bash scripts/rebuild-relaunch-consumer-mac-app.sh --instance gui-verify-20260508` passed bundle verification/opened app; Gatekeeper rejected because it was an unnotarized debug smoke.                                                                              | Do not use this as GUI Channels proof; TCC blocked `get_app_state` for the debug app.                     |
...
1. Unblock Computer Use/TCC, then rerun the Channels tab smoke. PR #645 fixed
   Telegram outbound activity telemetry, #650 fixed stale entrypoint mismatch
   detection, and #651 fixed packaged runtime root preservation during gateway
   install/restart. The current blocker is GUI automation access:
   `get_app_state` for `ai.openclaw.consumer.mac.debug.gui-verify-20260508`
   returned Apple event error `-10000`, `Sender process is not authenticated`.
2. Recut the next public artifact from current `main`/newer before broad
   ...
   artifacts do not contain #645/#650/#651. Sparkle/final public artifacts must
   be rebuilt from current `main` at `f238261852` or newer after GUI proof, not
   older `5515`/`759`/`4d01` builds.
...
- LaunchAgent proof:
  ...
  `{"ok":true,"status":"live"}`.
- Remaining gate:
  GUI Channels proof remains blocked by Computer Use/TCC. `get_app_state` for
  `ai.openclaw.consumer.mac.debug.gui-verify-20260508` returned Apple event
  error `-10000`, `Sender process is not authenticated`.
- Release caveat: public release lane remains separate. Public artifacts must
  be recut from current `main` at `f238261852` or newer after GUI proof, not
  older `5515`/`759`/`4d01` builds.
...
- Stop describing the Channels tab blocker as only an unimplemented verifier,
  ...
  activity gap fix, #650 merged stale entrypoint mismatch detection, and #651
  merged packaged-root preservation. The latest blocker is Computer Use/TCC for
  literal GUI Channels proof. `/healthz` and LaunchAgent proof are necessary
  but not sufficient for a GUI pass.
