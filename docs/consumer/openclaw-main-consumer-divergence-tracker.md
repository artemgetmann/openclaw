...
1. Run an interactive Sparkle UI smoke if the product claim needs the exact
   dialog path. Deterministic Sparkle update completion from `v2026.3.14` to
   `v2026.3.15` already passed.
2. Run a visual GUI smoke for the SwiftUI Channels tab `Verify first task`
...
- Sparkle caveat: interactive dialog visual proof remains open; loaded launchd env still reported `OPENCLAW_SERVICE_VERSION => 2026.3.14` even though the app and packaged entrypoint were updated.
