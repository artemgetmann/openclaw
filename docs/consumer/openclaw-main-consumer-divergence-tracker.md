...
| Overlay/defaults policy               | Mostly completed | Core runtime/setup is shared. Telegram `/model` starts with Claude, ChatGPT, and More. Fresh consumer configs get broad useful bundled skills by default, and model-facing skills remain visible when setup/auth/bins are missing.                        | Keep future onboarding presentation defaults explicit instead of scattering product conditionals.         |
...
1. Verify the simplified Telegram model picker on the live Jarvis bot.
2. Finish any remaining onboarding presentation defaults.
3. Shrink branch/docs debt last.
