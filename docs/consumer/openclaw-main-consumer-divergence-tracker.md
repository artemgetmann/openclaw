...
| Transitional Consumer packaging from main | Completed        | Main can build `OpenClaw Consumer.app`, `.zip`, `.dmg`, and dSYM from `origin/main`. Codesign stale-signature failure was fixed. Packaging now copies DMG/ZIP/dSYM ZIP handoff artifacts to the main checkout by default. Sparkle consumer release gates are in place. | Keep using this artifact only until rename migration is implemented.                                      |
