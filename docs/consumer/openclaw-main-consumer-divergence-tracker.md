...
1. Recut the next public artifact from current `main`/newer before broad
   ...
   artifacts do not contain #645/#650/#651/#653. Sparkle/final public artifacts
   must be rebuilt from current `main` at `849514bacc` or newer, not older
   `5515`/`759`/`4d01`/`f238` builds.
...
3. Keep account/license/backend work and public package/secrets audit open.
...
- Release caveat: public release lane remains separate. Public artifacts must
  be recut from current `main` at `849514bacc` or newer, not older
  `5515`/`759`/`4d01`/`f238` builds.
...
- Remaining blocker: public release provenance only. Sparkle/notarized artifacts
  need to be recut from current `main` or newer; this local installed-app smoke
