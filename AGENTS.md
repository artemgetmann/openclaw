...
- Before any local command that can run longer than 30 seconds, start workers, build/package an app, run a broad test/review suite, or perform browser/GUI E2E, read `docs/agent-guides/fleet-resource-control.md`. For ordinary agent-owned work, wrap the actual command in `scripts/with-dedicated-agent-slot.sh`; its default bounded wait holds no lease before admission and executes the workload exactly once when admitted. Do not run a separate `--check`, duplicate a yielded waiter, or model-poll its session. If the client yields before completion, preserve the exact transaction and use only a continuation route proven for that task; never claim the shell alone will wake a finished Codex turn. Stop on `guard_internal`, authorization failure, runtime health termination, bounded-wait expiry, or an unproved continuation boundary and report that exact terminal condition. Never run more than one heavy local command across worktrees; prefer remote CI for full suites.
- For every implementation PR, follow the canonical builder -> tester -> release-worker lifecycle in `docs/agent-guides/workflow.md`. The builder owns source, review fixes, and CI readiness but never merges; after exact-head tester `PASS`, a distinct fenced queue lease owns repo-backed release execution. Native release tasks are optional coordination; the documented direct rollback still requires one fresh user-visible release task and exact builder archival. When the user says `ship this end-to-end`, keep that lifecycle moving without returning routine coordination to the user and preserve explicit task authority in the release packet. Package, deploy, restart, install, shared-runtime mutation, credentials, destructive cleanup, and live/external actions retain their explicit authority boundaries.
- Use `nested-read-only` only for short, deterministic, immutable-head read-only validation with no protected resource, external effect, cleanup duty, long wait, user decision, or durable-transcript need. For other testing, use either a receipt-bound `delegated-worker` or a fresh user-visible project-scoped tester task, as the user and lifecycle contract require. A delegated worker must have a distinct exact identity, isolated immutable-head environment, explicit protected/live authority, terminal receipt, and cleanup proof; it is not the builder testing its own work. Never use `nested-read-only` for live/external testing or a direct rollback release worker. Repo-backed release executors are identified and fenced by the durable queue, not native task type. Before either delegation—even after context compaction—run `scripts/pr-lifecycle handoff-test` or `scripts/pr-lifecycle handoff-release`, then consume its emitted transport contract exactly as required by `docs/agent-guides/workflow.md`.
