...
- Use a nested tester only for short, deterministic, immutable-head read-only validation with no protected resource, external effect, cleanup duty, long wait, user decision, or durable-transcript need. Otherwise use a fresh user-visible project-scoped tester task and archive that exact task after recording its receipt. Never use a nested sub-agent for a live/external tester or a direct rollback release worker. Repo-backed release executors are identified and fenced by the durable queue, not native task type. Before either delegation—even after context compaction—run `scripts/pr-lifecycle handoff-test` or `scripts/pr-lifecycle handoff-release`, then consume its emitted transport contract exactly as required by `docs/agent-guides/workflow.md`.
