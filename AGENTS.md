...
- For every implementation PR, follow the canonical builder -> tester -> release-worker lifecycle in `docs/agent-guides/workflow.md`. The builder owns source, review fixes, and CI readiness but never merges; the fresh user-visible release task archives the exact builder at accepted handoff, then owns the normal merge after exact-head tester `PASS`. When the user says `ship this end-to-end`, keep that lifecycle moving without returning routine coordination to the user and preserve explicit task authority in the release packet. Package, deploy, restart, install, shared-runtime mutation, credentials, destructive cleanup, and live/external actions retain their explicit authority boundaries.
