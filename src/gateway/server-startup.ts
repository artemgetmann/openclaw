...
export async function startGatewaySidecars(params: {
  ...
  const skipSessionLockCleanup =
    consumerMinimalStartup || isTruthyEnvValue(process.env.OPENCLAW_DEBUG_SKIP_SESSION_LOCK_CLEANUP);
  ...
  const skipInternalHookLoading =
    consumerMinimalStartup || isTruthyEnvValue(process.env.OPENCLAW_DEBUG_SKIP_INTERNAL_HOOK_LOADING);
  ...
  const skipMemoryBackendStartup =
    consumerMinimalStartup || isTruthyEnvValue(process.env.OPENCLAW_DEBUG_SKIP_MEMORY_BACKEND_STARTUP);
