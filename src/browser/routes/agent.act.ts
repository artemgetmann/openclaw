      ...
      run: async ({ profileCtx, cdpUrl, tab }) => {
        ...
        switch (kind) {
          ...
          case "chooseOption": {
            ...
            const timeoutMs = toNumber(body.timeoutMs);
            if (isExistingSession) {
              const target = selector ? "selector" : "ref";
              const targetValue = selector ?? ref!;
              const fn =
                target === "selector"
                  ? `() => (${buildExistingSessionChooseOptionScript(target)})(${JSON.stringify(
                      targetValue,
                    )}, ${JSON.stringify(optionText)}, ${JSON.stringify(match)}, ${JSON.stringify(
                      query,
                    )}, ${JSON.stringify(String(timeoutMs ?? 10_000))})`
                  : `(targetOrControl) => (${buildExistingSessionChooseOptionScript(target)})(
                      targetOrControl,
              ...
              const result = await evaluateChromeMcpScript({
                ...
                targetId: tab.targetId,
                fn,
                args: target === "ref" ? [targetValue] : undefined,
                timeoutMs: timeoutMs ?? undefined,
              });
