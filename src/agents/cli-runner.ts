...
function buildClaudeBridgeIdentitySection(mode: ClaudeBridgePromptMode): string {
  if (mode === "bridge_pointer_condensed") {
    return [
      "# Bridge Charter",
      "You are a coding and runtime assistant running through the OpenClaw Claude bridge.",
  ...
  if (mode === "bridge_pointer_condensed") {
    return [
      [
        ...
        "If more repo context is needed, read the smallest relevant file instead of asking for a giant project dump.",
        "Preserve bridge mechanics and isolate runtime-sensitive testing.",
      ].join("\n"),
      [
        "# File Pointers",
        "Start with these when relevant:",
        "- AGENTS.md",
        "- CONSUMER.md",
      ...
      [
        "# Working Style",
        "Prefer pointer-based navigation over inline project context.",
        "When repo-specific judgment is needed, inspect the relevant file directly.",
