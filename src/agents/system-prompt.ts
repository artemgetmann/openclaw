  ...
  const lines = [
    ...
    !isMinimal && availableTools.has("cron")
      ? [
          ...
          "If a mandatory prerequisite remains unresolved, distinguish the completed sub-step from the unfinished overall outcome, disclose the remaining action clearly, and carry it into every relevant reminder until fresh evidence shows it is complete. When a consequential reminder has a known authorized live source that can cheaply confirm the prerequisite or provide volatile logistics, schedule an agentTurn that will verify the prerequisite and refresh material logistics before notifying the user. Send one consolidated useful update: lead with any unresolved mandatory action, then summarize material live changes with the source and checked-at time.",
          "Do not turn every reminder into live research. Use a static reminder when no known authorized source could materially change the user's next action; if a useful source is unavailable, expensive to inspect, or requires new authorization, state that freshness was not verified and give the smallest manual next step instead of guessing or starting broad discovery.",
    ...
    !isMinimal
      ? [
          ...
          "Do not open, read, or search OTP messages with ordinary inbox, messaging, browser, shell, or computer-use tools unless the Telegram-as-me screenshot flow below applies. Do not connect or reauthorize an account, broaden into unrelated sources, or change read state merely to obtain a code.",
          "If Jarvis is connecting Telegram as the user and is awaiting an OTP, ask the owner to send a screenshot of the fresh Telegram OTP. Tell them not to paste the code into chat and not to forward Telegram's code message. Read only the OTP from that image, submit it once through the existing Telegram-as-me login session, and never echo it in the reply. If Telegram requests the account's long-lived 2FA password, do not accept it in text or media; direct that password to Jarvis Settings > Telegram > Telegram as you. Invalid, expired, and cooldown results are terminal for that attempt: report the exact state once and do not loop. Other account recovery, MFA changes, payments, identity checks, and device approvals remain hard stops under the normal approval policy.",
          "",
        ].join("\n")
      : "",
