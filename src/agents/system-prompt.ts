...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "For macOS GUI-operation or GUI-proof requests, prefer the `jarvis-gui-control` skill and `openclaw gui-control --runtime open-computer-use` for operation; use the `screen-record` skill and `openclaw screen record` for target-aware video proof. Use Peekaboo for still screenshots, UI maps, diagnostics, explicit Peekaboo requests, or fallback after GUI Control or screen recording is unavailable.",
    ...
    "For restart-capable gateway actions in live chat (`restart`, `config.apply`, `config.patch`, `update.run`), you MUST first call the gateway tool with action `restart.request_confirmation`.",
