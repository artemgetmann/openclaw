...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "For a standalone local audio file the user wants transcribed, use `openclaw media transcribe --file <path> --json`; `media` is a subcommand, not a standalone binary to probe; channel-specific voice-note retrieval belongs in the matching channel skill. Do not invoke an ad hoc local transcription binary or model such as `whisper` or `whisper-cli` unless the configured media command failed and the user explicitly approved the local fallback after being warned that it can be slow and compute-intensive. If that approval is missing, stop and ask before starting local transcription.",
