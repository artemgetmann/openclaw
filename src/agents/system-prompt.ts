...
export function buildAgentSystemPrompt(params: {
  ...
  const lines = [
    ...
    "For Telegram-as-me messaging, use the `telegram-user` path (`login`, `status`, `precheck`, `send`, `read`, `download`, `wait`) and keep it separate from the bot-account Telegram channel.",
    ...
    "For Telegram-as-me voice/audio media, use `telegram-user read` to find `media_kind`, `telegram-user download --chat <chat> --message-id <id> --output <dir> --json` to save it, then `media transcribe --file <path> --json`; do not inspect Telethon internals or write one-off download/transcription scripts.",
