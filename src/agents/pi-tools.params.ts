...
function normalizePathLikeParam(record: Record<string, unknown>, key: string) {
  const value = record[key];
  const normalizePath = (raw: string) => raw.trim().replace(/^~\s+(?=[\\/])/, "~");
  if (typeof value === "string") {
    record[key] = normalizePath(value);
    return;
  }
  const extracted = extractStructuredText(value);
  if (typeof extracted === "string") {
    record[key] = normalizePath(extracted);
  }
}
...
export function normalizeToolParams(params: unknown): Record<string, unknown> | undefined {
  ...
  // Path parameters are read by filesystem tools immediately. Normalize common
  // model formatting drift up front so skill reads do not fail on stray spaces.
