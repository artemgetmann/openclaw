...
describe("runCliAgent with process supervisor", () => {
  ...
  it("streams codex-cli JSONL deltas through partial reply callbacks", async () => {
    const stdout = [
      JSON.stringify({ type: "thread.started", thread_id: "codex-thread-1" }),
      JSON.stringify({ type: "agent_message_delta", delta: "Hello" }),
      JSON.stringify({ type: "agent_message_delta", delta: " there" }),
    ...
    const result = await runCliAgent({
      ...
      model: "gpt-5.2-codex",
      timeoutMs: 1_000,
      runId: "run-codex-stream",
      onAssistantMessageStart,
      onPartialReply,
    });

    expect(result.payloads?.[0]?.text).toBe("Hello there");
    expect(result.meta.agentMeta?.sessionId).toBe("codex-thread-1");
    expect(onAssistantMessageStart).toHaveBeenCalledTimes(1);
    expect(onPartialReply.mock.calls).toEqual([[{ text: "Hello" }], [{ text: "Hello there" }]]);
  });

  it("streams resumed codex-cli JSONL deltas through partial reply callbacks", async () => {
    const stdout = [
      JSON.stringify({ type: "thread.started", thread_id: "codex-thread-1" }),
      JSON.stringify({ type: "agent_message_delta", delta: "Welcome" }),
      JSON.stringify({ type: "agent_message_delta", delta: " back" }),
      JSON.stringify({ type: "agent_message", message: "Welcome back" }),
    ].join("\n");
    supervisorSpawnMock.mockImplementationOnce(
      async (input: { onStdout?: (chunk: string) => void }) => {
        input.onStdout?.(`${stdout}\n`);
    ...
    const result = await runCliAgent({
      ...
      model: "gpt-5.2-codex",
      timeoutMs: 1_000,
      runId: "run-codex-resume-stream",
      cliSessionId: "codex-thread-1",
      onAssistantMessageStart,
      onPartialReply,
    });

    expect(result.payloads?.[0]?.text).toBe("Welcome back");
    expect(onAssistantMessageStart).toHaveBeenCalledTimes(1);
    expect(onPartialReply.mock.calls).toEqual([[{ text: "Welcome" }], [{ text: "Welcome back" }]]);
    const spawnInput = supervisorSpawnMock.mock.calls[0]?.[0] as { argv?: string[] };
    expect(spawnInput.argv).toEqual(
      expect.arrayContaining(["codex", "exec", "resume", "codex-thread-1", "--json"]),
    );
  });

  it("falls back to text resume when codex-cli rejects JSONL resume flags", async () => {
    supervisorSpawnMock
    ...
    const result = await runCliAgent({
      ...
      model: "gpt-5.2-codex",
      timeoutMs: 1_000,
      runId: "run-codex-resume-fallback",
      cliSessionId: "codex-thread-1",
    });

    expect(result.payloads?.[0]?.text).toBe("fallback ok");
    expect(supervisorSpawnMock).toHaveBeenCalledTimes(2);
    const firstSpawn = supervisorSpawnMock.mock.calls[0]?.[0] as { argv?: string[] };
    const secondSpawn = supervisorSpawnMock.mock.calls[1]?.[0] as { argv?: string[] };
    expect(firstSpawn.argv).toContain("--json");
    expect(secondSpawn.argv).not.toContain("--json");
  });
