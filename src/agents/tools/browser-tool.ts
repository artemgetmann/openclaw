...
export function createBrowserTool(opts?: {
  ...
  return {
    ...
    description: [
      ...
      "For searchable selects/comboboxes, use the Codex pattern: snapshot, act kind=fill on the combobox/search input ref with the visible option text, then act kind=press key=Enter on the same ref when available, with includeSnapshot=true after the mutating step.",
      "Use act kind=select for native <select> controls. For custom listbox/combobox controls, use fill+press Enter first and reserve evaluate/script for fallback after refs and keyboard selection fail.",
