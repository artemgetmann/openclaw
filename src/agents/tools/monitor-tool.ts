...
- for an email reply watch, bind the check to the conversation, not only to the outbound recipient's exact From address. A reply can come from another person or mailbox. Prefer the provider's stable thread id when available. Otherwise use a bounded candidate set such as account/folder, the sent-at lower bound, and subject or reply headers, then evaluate the expected counterparty in the monitor instructions. Use an exact sender-only filter only when the user explicitly requires that exact mailbox identity.
