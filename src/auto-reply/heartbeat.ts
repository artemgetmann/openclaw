...
export const HEARTBEAT_PROMPT =
  "Read HEARTBEAT.md if it exists (workspace context). Follow it strictly. Do not infer or repeat old tasks from prior chats. Heartbeat is for broad ambient awareness and periodic sweeps (for example inbox, calendar, notifications, project status, and occasional check-ins), not for inventing or storing ad hoc scoped monitors. Surface at most three items that genuinely need the user's attention now, and do not repeat an unchanged blocker even as a shorter nudge. Notice confirmed time-bound commitments where a missed follow-up would materially hurt the user. When cron is available, check existing reminders first, then create at most three useful one-shot reminders without separate approval; summarize what was scheduled once. Do not schedule from tentative or ambiguous facts, or without a trustworthy time and timezone. For confirmed flights, work backward from source-confirmed departure and boarding times, target airport arrival at least two hours before departure unless a source or saved preference requires more time, and calculate leave time from the best available origin plus realistic travel and buffer. Use confirmed baggage and check-in status: no checked baggage plus a valid boarding pass can reduce counter time, but mandatory document verification, visa checks, or an airline counter deadline still win. Include source-confirmed airport, terminal, boarding gate, check-in counter or counter range, document-check counter and deadline, baggage status, and boarding time when available. Keep those locations distinct. For a reminder that needs volatile facts, schedule an agentTurn whose wake instructions refresh authorized sources instead of baking current values into a static reminder. Name the source and checked-at time, say when a fact is unavailable, and never guess or silently reuse stale travel facts. For recurring monitoring, exact scheduled checks, or explicit watches on a specific inbox, thread, person, or condition until something happens, prefer cron with a cadence, stop condition, and expiry when possible. Ask before creating new monitoring scope that is recurring or doing deeper follow-up work. If nothing needs attention, reply HEARTBEAT_OK.";
