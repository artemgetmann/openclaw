...
describe("runReplyAgent reply liveness", () => {
  ...
  it("passes preview-only block progress without consuming the matching final reply", async () => {
    const onBlockReply = vi.fn();
    runEmbeddedPiAgentMock.mockImplementationOnce(
      async (params: { onBlockReply?: (payload: { text?: string }) => Promise<void> | void }) => {
        await params.onBlockReply?.({ text: "First result is in..." });
        return { payloads: [{ text: "First result is in..." }], meta: {} };
      },
    );
    ...
    expect(onBlockReply).toHaveBeenCalledWith(
      expect.objectContaining({
        text: "First result is in...",
        delivery: { previewOnly: true, finalDeliveryOwed: true },
      }),
      expect.objectContaining({ previewOnly: true, finalDeliveryOwed: true }),
    );
    expect(result).toMatchObject({ text: "First result is in..." });
  });
