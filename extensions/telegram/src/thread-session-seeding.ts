...
function applySeededFutureThreadDefaults(params: {
  ...
  const parentProvider =
    typeof parentEntry?.futureThreadProviderOverride === "string"
      ? parentEntry.futureThreadProviderOverride.trim()
      : "";
  const parentModel =
    typeof parentEntry?.futureThreadModelOverride === "string"
      ? parentEntry.futureThreadModelOverride.trim()
      : "";
  const parentThinking =
    typeof parentEntry?.futureThreadThinkingLevelOverride === "string"
      ? parentEntry.futureThreadThinkingLevelOverride.trim()
      : "";
  ...
  if (parentProvider && parentModel) {
    applyModelOverrideToSessionEntry({
      entry,
      selection: {
        provider: parentProvider,
        model: parentModel,
      } satisfies ModelOverrideSelection,
    });
  }
  if (parentThinking) {
    entry.thinkingLevel = parentThinking;
