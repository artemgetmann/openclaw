...
export const buildTelegramMessageContext = async ({
  ...
  const directConversationAllowFrom = senderId
    ? [senderId]
    : [String(msg.from?.username ?? chatId).trim()].filter(Boolean);
  const contextAllowFrom = isGroup
    ? normalizeTrustEntries(groupAllowOverride ?? groupAllowFrom)
    : normalizeTrustEntries(directConversationAllowFrom);
  ...
  const { ctxPayload, skillFilter } = await buildTelegramInboundContextPayload({
    ...
    transcript: bodyResult.transcript,
    suppressCurrentMediaForAgent: bodyResult.suppressCurrentMediaForAgent,
  ...
  return {
    ...
    route,
    skillFilter,
    handledDirectReplyText: bodyResult.handledDirectReplyText,
    sendTyping,
    sendRecordVoice,
    ackReactionPromise,
    reactionApi,
