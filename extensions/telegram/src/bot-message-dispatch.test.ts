...
describe("dispatchTelegramMessage draft streaming", () => {
  ...
  it("streams assistant-authored progress blocks into the answer preview", async () => {
  ...
  it("keeps final text in the payload when TTS voice follows a progress preview", async () => {
    const draftStream = createSequencedDraftStream(2101);
    createTelegramDraftStream.mockReturnValue(draftStream);
    dispatchReplyWithBufferedBlockDispatcher.mockImplementation(async ({ dispatcherOptions }) => {
      await dispatcherOptions.deliver(
        {
          text: "Preparing the final voice reply...",
          delivery: { previewOnly: true, finalDeliveryOwed: true },
        },
        { kind: "block" },
      );
      await dispatcherOptions.deliver(
        {
          ...
          mediaUrl: "https://example.com/final.ogg",
          audioAsVoice: true,
        },
        { kind: "final" },
      );
    ...
    expect(draftStream.update).toHaveBeenCalledWith("Preparing the final voice reply...");
    expect(deliverReplies).toHaveBeenCalledWith(
      expect.objectContaining({
        replies: [
          expect.objectContaining({
