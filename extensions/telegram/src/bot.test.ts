...
describe("createTelegramBot", () => {
  ...
  it("stores future-thread defaults when selecting a model inside a DM topic", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    editMessageTextSpy.mockClear();
    ...
    try {
      ...
      await callbackHandler({
        callbackQuery: {
          ...
          data: "mdl_sel_openai-codex/gpt-5.3-codex",
          from: { id: 9, first_name: "Ada", username: "ada_bot" },
          message: {
            chat: { id: 1234, type: "private" },
            date: 1736380800,
      ...
      expect(store["agent:main:main:thread:1234:42"]).toEqual(
        expect.objectContaining({
  ...
  it("stores future-thread defaults when selecting a model inside a forum topic", async () => {
    onSpy.mockClear();
    replySpy.mockClear();
    editMessageTextSpy.mockClear();

    const storePath = `/tmp/openclaw-telegram-model-forum-topic-${process.pid}-${Date.now()}.json`;
    ...
    try {
      ...
      const store = loadSessionStore(storePath, { skipCache: true });
      expect(store["agent:main:telegram:group:-1001234567890:topic:77"]).toEqual(
        expect.objectContaining({
          providerOverride: "openai-codex",
          modelOverride: "gpt-5.3-codex",
        }),
      );
      expect(store["agent:main:telegram:group:-1001234567890"]).toEqual(
        expect.objectContaining({
          futureThreadProviderOverride: "openai-codex",
          futureThreadModelOverride: "gpt-5.3-codex",
        }),
      );
      expect(answerCallbackQuerySpy).toHaveBeenCalledWith("cbq-model-forum-topic-1");
    } finally {
      await rm(storePath, { force: true });
    }
  });

  it("pre-seeds DM topic sessions on topic creation so older topics do not inherit newer defaults", async () => {
    onSpy.mockClear();
    replySpy.mockClear();

    const storePath = `/tmp/openclaw-telegram-dm-topic-seed-${process.pid}-${Date.now()}.json`;
    ...
    try {
      ...
      await handler({
        message: {
          ...
          message_thread_id: 314,
          is_topic_message: true,
          text: "hello from an older topic",
          from: { id: 42, first_name: "Ada" },
        },
        me: { username: "openclaw_bot" },
        getFile: async () => ({ download: async () => new Uint8Array() }),
      ...
      const store = loadSessionStore(storePath, { skipCache: true });
      expect(store["agent:main:main:thread:1234:314"]).toEqual(
        expect.objectContaining({
          providerOverride: "openai-codex",
          modelOverride: "gpt-5.3-codex",
          thinkingLevel: "off",
